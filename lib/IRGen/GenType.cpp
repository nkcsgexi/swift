...
namespace {
  ...
  /// client's view (encap-c-decl). Layout is recovered from the defining
  /// module's HiddenTypeLayouts table; under the V1 plain-C assumption the
  /// type is trivially copyable and trivially destructible, so value witnesses
  /// reduce to memcpy / no-op.
  ///
  ...
  /// always passed indirectly. Reports 0 extra inhabitants for the same
  /// reason — the spare-bit pattern of the underlying scalar layout is not
  /// available without the real type.
  class HiddenTypeInfo final
      : public IndirectTypeInfo<HiddenTypeInfo, FixedTypeInfo> {
  public:
    HiddenTypeInfo(llvm::Type *storage, Size size, Alignment align)
...
#include "swift/AST/ReferenceStorage.def"
  ...
  case TypeKind::Hidden: {
    ...
    return new HiddenTypeInfo(storage, Size(layout->size),
